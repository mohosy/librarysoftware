    ...
    if (person != nullptr) {
        ...
        if (person->getName() == name) {
            ...
            for (int i = 0; i < person->getBorrowed(); i++) {
                ...
                if (book != nullptr) {
                    cout << "* " << removeUnderscores(book->getTitle()) << " (" << book->getCode() << ") - " << book->getExtraInfo() << endl;
                }
            }
        } else {
        ...
        while (!exit) {
            ...
            switch (response) {
                ...
                case 2: {
                    ...
                    int maxBooksAllowed = 2; // Default for students
                    if (dynamic_cast<Teacher*>(person)) {
                        maxBooksAllowed = 3; // For teachers
                    }

                    if (person->getBorrowed() >= maxBooksAllowed) {
